 ...
 * comes from same family, means implementing same interface? You can restrict the generics type parameter to a certain
 * group of objects which implements same interface. You can achieve this my specifying extends <interface-name> at
 * class definitions, look at the example, it gives you more comments to understand. You can also specify multiple
 * interfaces at the definision. you can do this by specifying mulitple interfaces seperated by "&". You can also
 * specify class which implements an interface and the interface together. For example: class MyClass<T extends
 * TestClass & TestInterface> { Look at example for more understanding. - See more at:
 * http://java2novice.com/java-generics/implements-interface/#sthash.wSoRGoxi.dpuf
 * @author java2novice
 */
